...
DEFAULT_ADVERTISEMENT_DETECTION_PROMPT = """Analyze the following transcript and identify advertising sections.
...
DEFAULT_TOPIC_EXTRACTION_PROMPT = """List the main topics covered in the following podcast transcript.
...
def load_prompts(
    ...
    try:
        ...
        return Prompts(
            ...
            content_summary=parsed_prompts.get(
                'content_summary', DEFAULT_CONTENT_SUMMARY_PROMPT
            ),
        ...
        if warn_on_fallback:
            _display_warning(
                f"Could not read prompts file {prompts_path}: {e}. "
                "Using built-in default prompts."
            )
        return Prompts.defaults()
        ...
        if warn_on_fallback:
            _display_warning(
                f"Error parsing prompts file {prompts_path}: {e}. "
                "Using built-in default prompts."
            )
        return Prompts.defaults()
