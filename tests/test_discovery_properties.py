    ...
    **Validates: Requirements 1.3, 2.3**
    ...
    def test_parse_feed_respects_limit(
        ...
        **Validates: Requirements 2.3**
        """
        with patch("podtext.services.rss.httpx.Client") as mock_client_class, \
    ...
    def test_parse_feed_entries_respects_limit(
        ...
        **Validates: Requirements 2.3**
        ...
        # Property: Results length SHALL be ≤ limit
        assert len(results) <= limit, (
            f"Parsed entries length {len(results)} exceeds limit {limit}"
        )
        ...
        **Validates: Requirements 2.1**
        ...
        for episode in results:
            ...
            assert episode.title, (
                f"Episode title should be non-empty, got: '{episode.title}'"
            )
    ...
    def test_parsed_episodes_have_valid_index(
        ...
        **Validates: Requirements 2.1**
        ...
        for i, episode in enumerate(results, start=1):
            ...
            assert episode.index == i, (
                f"Episode index should be {i}, got: {episode.index}"
            )
            assert episode.index > 0, (
                f"Episode index should be positive, got: {episode.index}"
            )
    ...
    def test_parsed_episodes_all_properties_valid(
        ...
        **Validates: Requirements 2.1**
        ...
        for episode in results:
            ...
            assert episode.index > 0, (
                f"Episode should have positive index, got: {episode.index}"
            )
        ...
        **Validates: Requirements 2.1**
        ...
        with patch("podtext.services.rss.httpx.Client") as mock_client_class, \
            ...
            for episode in results:
                ...
                # Valid media URL
                assert episode.media_url, (
                    f"Episode should have non-empty media_url"
                )
                parsed_url = urlparse(episode.media_url)
                assert parsed_url.scheme in ("http", "https") and parsed_url.netloc, (
                    f"Episode should have valid media URL, got: '{episode.media_url}'"
